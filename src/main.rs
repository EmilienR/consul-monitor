    ...
    match c.service(service_name, tag.as_deref(), false, None) {
        Ok(consul_srvc_res) => {
            ...
            if cmin.is_some() && passing_service_count <= cmin.unwrap() {
                ...
                print!(
                    "OK : {} passing {} service instances",
                    passing_service_count, service_name
                );
                res = EXIT_OK;
            }
...
fn print_health_checks_perfdata_and_details(
    service_name: Option<String>,
    check_id: Option<String>,
    passing_check_count: u32,
    health_checks: &Vec<HealthCheck>,
) {
    println!("|passing_check_count={}", passing_check_count);
    let service_name = service_name.unwrap_or_else(|| "None".to_owned());
    let check_id = check_id.unwrap_or_else(|| "None".to_owned());
...
fn check_node_service_health(
    ...
    match c.node(node, check_id.as_deref(), service.as_deref(), None, None) {
        Ok(srvc_health) => {
            ...
            print_health_checks_perfdata_and_details(
                service,
                check_id,
                passing_check_count,
                &srvc_health.0,
        ...
        Err(err) => {
            error_exit(
                &format!("Failed to get service health on node {}", node),
                err,
            );
        }
    }
    ...
    match c.leader(None) {
        Ok(leader) => {
            if let Some(mut el) = expected_leader {
                ...
                if el != leader.0 {
                    println!(
                        "{} is not the expected cluster leader (expected {})",
                        el, leader.0
                    );
                    exit(EXIT_CRITICAL);
                }
...
fn main() {
    ...
    unsafe {
        ...
        ap.refer(&mut host)
            .add_option(&["-h", "--host"], StoreOption, "Consul service host");
        ...
        ap.refer(&mut token)
            .add_option(&["--token"], StoreOption, "Consul token");
        ap.refer(&mut warning_min).add_option(
            &["--warning-min"],
            StoreOption,
            "Warning if less than that value",
        );
        ap.refer(&mut warning_max).add_option(
            &["--warning-max"],
            StoreOption,
            "Warning if more than that value",
        );
        ap.refer(&mut critical_min).add_option(
            &["--critical-min"],
            StoreOption,
            "Critical if less than that value",
        );
        ap.refer(&mut critical_max).add_option(
            &["--critical-max"],
            StoreOption,
            "Critical if more than that value",
        );
        ap.refer(&mut service)
            .add_option(&["--service"], StoreOption, "Service name");
        ap.refer(&mut tag)
            .add_option(&["--tag"], StoreOption, "Service tag");
        ap.refer(&mut node)
            .add_option(&["--node"], StoreOption, "Node name");
        ap.refer(&mut check_id)
            .add_option(&["--check-id"], StoreOption, "CheckID");
        ap.refer(&mut expected_leader).add_option(
            &["--expected-leader"],
            StoreOption,
            "Expected cluster leader",
        );
        ap.refer(&mut expected_peer_count).add_option(
            &["--expected-peer-count"],
            StoreOption,
            "Expected peer count in cluster",
        );
        ap.refer(&mut CRITICAL_ON_ERROR).add_option(
            &["--critical-on-error"],
            StoreTrue,
            "Exit with critical status on error",
        );
        ap.refer(&mut VERBOSE)
            .add_option(&["--verbose"], StoreTrue, "Print extended output");
    ...
    let expected_peer_count =
        expected_peer_count.and_then(|x| if x < 0 { None } else { Some(x as usize) });
    ...
    let config = Config::new_from_consul_host(
        format!("http://{}", host.unwrap_or_else(|| "127.0.0.1".to_owned())).as_ref(),
        port,
        token,
    )
    .expect("Impossible de générer la configuration");
    ...
    match &*mode {
        ...
        _ => {
            eprintln!("Unknown check mode '{mode}'");
            exit(EXIT_UNKNOWN)
        }
    }
}
